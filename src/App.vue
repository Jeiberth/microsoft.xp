  ...
  <div class="custom-overlay">
    ...
    <div class="middle-section">
        ...
        <div class="instructions">
          <i class="fas fa-mouse-pointer"></i> Click on the user card to continue <router-view></router-view>
        </div>
    </div>
    <div class="border-section"></div>
    ...
    <div v-if="showStartMenu" class="start-menu" @click.stop>
      ...
      <div class="start-menu-content">
        ...
        <div class="right-column-l">
          <a :key="cv.id" class="menu-item" href="./Jeiberth_Malagon_FullStack_AI_Developer_Resume.pdf" download>
            <img :src="cv.icon" :alt="cv.name" class="menu-item-icon" />
            <span class="menu-item-label">{{ cv.name }}</span>
          </a>
...
const cv = ref({
  id: 'cv',
  name: 'My Resume',
  icon: new URL('@/assets/mydocuments.png', import.meta.url).href
});
...
const appDesktop = reactive([
  ...
  },
  {
    "id": 2,
    "name": "Aicook",
    "icon": new URL('@/assets/aicook.png', import.meta.url).href,
    "data": {
      "icon": new URL('@/assets/aicook.png', import.meta.url).href,
      "title": "Lead Ai Product Developer",
      "url": "https://aicook.info/",
      "text": "Engineered an AI-powered recipe generation platformn with Angular, Ionic and NgRx that dynamically creates personalized, multi-step cooking instructions from user-provided ingredients. Leveraged LangChain for custom prompt orchestration and large language model (LLM) integration, and optimized natural language understanding and recipe generation through the OpenAI API. Developed and integrated the backend using Laravel for real-time processing, ensuring seamless user interaction and system scalability",
      "skills": [
        new URL('@/assets/openai-icon.png', import.meta.url).href,
        new URL('@/assets/langchain-icon.png', import.meta.url).href,
        new URL('@/assets/prompt-icon.png', import.meta.url).href,
        new URL('@/assets/ngrx-icon.png', import.meta.url).href,
        new URL('@/assets/angular-icon.png', import.meta.url).href,
        new URL('@/assets/laravel-icon.png', import.meta.url).href,
        new URL('@/assets/browser-icon.png', import.meta.url).href,
        new URL('@/assets/mysql-icon.png', import.meta.url).href,
        new URL('@/assets/axios_icon.png', import.meta.url).href,
...
onMounted(() => {
  ...
  // setViewportHeight();
    ...
    // Also set a CSS custom property for full height
  ...
  window.addEventListener('orientationchange', () => {
    // Small delay to let Safari finish its UI adjustments
    setTimeout(setViewportHeight, 100);
  });
  
  // Safari-specific: update when viewport changes due to scrolling
...
/* Responsive design - FIXED: Keep horizontal layout on mobile */
@media (max-width: 768px) {
    .content-container {
        /* Keep horizontal layout, just scale down */
        width: 95%;
        transform: scale(0.8);
    }
    
    .left-column, .right-column {
        /* Reduce padding but maintain horizontal arrangement */
        padding: 15px;
    }
    
    /* Keep vertical line vertical, just make it shorter */
    .vertical-line {
        left: 50%;
        top: 10%;
        bottom: 10%;
    ...
    .user-container {
        /* Scale down but keep horizontal */
        padding: 10px;
    }
...
/* FIXED: Desktop app layout using flexbox instead of columns */
.main-page {
    flex: 1;
    background-image: url('@/assets/bg.webp');
    background-size: cover;
    ...
    position: relative;
    padding: 20px;
    padding-bottom: 60px; /* Extra space for taskbar */
    overflow-y: auto;
    overflow-x: hidden;
    
    /* Use flexbox with wrapping for better control */
    display: flex;
    flex-wrap: wrap;
...
.desktop-app {
    ...
    justify-content: flex-start;
    width: 120px;
    height: 100px; /* Fixed height to prevent content overflow */
    cursor: pointer;
    border-radius: 3px;
    ...
    /* Prevent breaking across columns/pages */
    break-inside: avoid;
...
.app-label {
    ...
    min-height: 2.2em;
    max-height: 3em !important; /* Prevent overflow */
}
...
.desktop-app.selected .app-label {
    ...
    border-radius: 2px;
    padding: 2px 4px;
    margin: 0 -4px; /* Negative margin to account for padding */
}

/* Responsive adjustments for desktop apps */
@media (max-width: 768px) {
    .main-page {
        padding: 15px;
        padding-bottom: 55px;
